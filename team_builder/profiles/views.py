...
class ProfileView(LoginRequiredMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['active_projects'] = self.get_object().user.project.filter(
            completed=False
        )
        context['past_projects'] = self.get_object().user.project.filter(
            completed=True
        )
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if profile_form.is_valid() and skills_formset.is_valid():
            ...
            for form in skills_formset:
                ...
                if skill_name:
                    skill = models.Skill.objects.get_or_create(
                        skill=skill_name)[0]
                    profile.skills.add(skill)
            profile.save()
            messages.add_message(
                self.request, messages.SUCCESS, "Profile updated successfully!"
            )
...
class NotificationsView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['new_notifications'] = self.get_queryset().filter(
            read=False).order_by('-created_at')
        context['old_notifications'] = self.get_queryset().filter(
            read=True).order_by('-created_at')
        return context
